...
final class HeaderBlock extends BlockBase implements ContainerFactoryPluginInterface
{
  ...
  public function build(): array
  {
    ...
    if ($top_logo_left instanceof \Drupal\file\FileInterface) {
      $left_logo_url = \Drupal::entityTypeManager()->getStorage('image_style')->load('large'); // Replace 'large' with your desired style
      $left_logo_url = $left_logo_url->buildUrl($top_logo_left-> getFileUri());
    }

    $right_logo_url = '';
    if ($top_logo_right instanceof \Drupal\file\FileInterface) {
      $right_logo_url = \Drupal::entityTypeManager()->getStorage('image_style')->load('large'); // Replace 'large' with your desired style
      $right_logo_url = $right_logo_url->buildUrl($top_logo_right->getFileUri());
    }
    $items = [
